# coding: utf-8

"""
    Graphiant APIs

    Graphiant API documentation.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from graphiant_sdk.models.mana_v2_prefix_set import ManaV2PrefixSet

class TestManaV2PrefixSet(unittest.TestCase):
    """ManaV2PrefixSet unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ManaV2PrefixSet:
        """Test ManaV2PrefixSet
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ManaV2PrefixSet`
        """
        model = ManaV2PrefixSet()
        if include_optional:
            return ManaV2PrefixSet(
                description = 'example string',
                entries = [
                    graphiant_sdk.models.mana_v2_prefix_set_entry.manaV2PrefixSetEntry(
                        id = 1234567891011, 
                        ip_prefix = 'example string', 
                        mask_lower = 123, 
                        mask_upper = 123, 
                        seq = 123, )
                    ],
                error_message = 'example string',
                global_id = 1234567891011,
                id = 1234567891011,
                mode = 'ENUM_VALUE',
                name = 'example string',
                policies = [
                    graphiant_sdk.models.mana_v2_prefix_set_policy.manaV2PrefixSetPolicy(
                        attach_point = 'ENUM_VALUE', 
                        id = 1234567891011, 
                        name = 'example string', )
                    ],
                policy_count = 123,
                status = 'ENUM_VALUE'
            )
        else:
            return ManaV2PrefixSet(
        )
        """

    def testManaV2PrefixSet(self):
        """Test ManaV2PrefixSet"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
